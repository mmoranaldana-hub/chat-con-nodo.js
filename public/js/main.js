...
function bindUI(){
  ...
  document.getElementById('avatar-file').addEventListener('change', async (e)=>{
    const f = e.target.files[0]; if(!f) return; const reader = new FileReader(); reader.onload = async ()=>{
      const dataUrl = reader.result; const r = await api('/me/profile','POST', { description: null, avatar: dataUrl }); if(r && r.ok){ document.getElementById('me-avatar-img').src = dataUrl; document.getElementById('me-avatar-img').style.display='block'; document.getElementById('me-avatar-initial').style.display='none'; }
...
async function loadContacts(){
  const cs = await api('/contacts'); const list = document.getElementById('contacts-list'); list.innerHTML='';
  const g = document.createElement('div'); g.className='item'; g.innerHTML = `<div style="width:40px"><div class="logo">G</div></div><div class="meta"><div class="title">Global</div><div class="small">Chat público</div></div>`; g.onclick = ()=> openGlobal(); list.appendChild(g);
...
async function openGlobal(){ current = { type:'global' }; document.getElementById('chat-name').textContent='Global'; document.getElementById('chat-sub').textContent='Chat público'; document.getElementById('messages').innerHTML=''; const msgs = await api('/messages'); if(Array.isArray(msgs)) msgs.forEach(render); scrollBottom(); socket.emit('join_group',0); }
...
function render(m){ const box = document.getElementById('messages'); const el = document.createElement('div'); const isMe = me && ((m.user_id && m.user_id===me.id) || (m.from && m.from===me.id)); el.className = 'msg ' + (isMe? 'me':'other'); const meta = document.createElement('div'); meta.className='meta'; meta.textContent = (m.username || m.fromName || ('User '+(m.from||m.user_id||''))) + ' • ' + new Date(m.created_at).toLocaleString(); const body = document.createElement('div'); body.textContent = m.text || ''; el.appendChild(meta); el.appendChild(body); box.appendChild(el); }
...
socket.on('group_member_added',(d)=>{ loadGroups(); });
socket.on('group_created',(d)=>{ loadGroups(); });
socket.on('friend_request',(d)=>{ if(me && d.toId === me.id){ alert('Nueva solicitud de amistad de ' + d.fromName); } });
